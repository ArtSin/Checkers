...
{
    class NegaScoutBotClient : BotClient
    {
        // Бесконечность
        ...
        public NegaScoutBotClient(int moveDelay, ulong seed, int maxDepth) : base(moveDelay, seed)
        {
        ...
        {
            Move maxMove = null;
            NegaScout(Color, maxDepth, -INF, INF, ref maxMove);
            return maxMove;
        }

        private int NegaScout(PlayerColor currPlayer, int depth, int alpha, int beta, ref Move maxMove)
        {
            ...
            foreach (var move in allMoves)
            {
                ...
                if (first)
                {
                    res = -NegaScout(1 - currPlayer, depth - 1, -beta, -alpha, ref maxMove);
                    first = false;
                }
                else
                {
                    res = -NegaScout(1 - currPlayer, depth - 1, -alpha - 1, -alpha, ref maxMove);
                    if (alpha < res && res < beta)
                        res = -NegaScout(1 - currPlayer, depth - 1, -beta, -res, ref maxMove);
                }
                if (res > result)
                {
